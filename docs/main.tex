...
Observemos en la figura \ref{grf:ex2} que tanto el mejor caso como el peor caso tienden a constantes, implicando que el crecimiento asintótico efectivamente es como esperábamos. Cabe destacar que, en particular para el peor caso, la función $NL + P = NL + 1+L+(N-2)L^2 \in O(L^3)$, pues $N = L$ implica que el último término es $O(L^3)$, y el primero es $O(L^2)$.
...
Para generar el mejor caso para este ejercicio, y que el $m$ sea el mismo para cada prueba que el $m$ en el peor caso, lo que hacemos es calcular a cada paso el $m$ correspondiente al peor caso, es decir, a un $K_n$ para un determinado $n$. Sin embargo, este $n$ no es la cantidad de nodos del grafo para el mejor caso, ya que lo que estamos midiendo es la complejidad sobre la cantidad de aristas. Como el mejor caso para Kruskal es que el grafo de entrada ya sea un árbol, lo que hacemos es generar un árbol unario de $m$ aristas, es decir, una cadena de nodos. Esto significa que excepto el primer y último nodo, que tienen grado 1, todos los demás nodos tienen grado 2, por lo que la cantidad de nodos está dada por:

$ 2M = 2 + \sum\limits_{i=0}^{N-2}{2} = 2 + (N-2) * 2 \rightarrow M = N - 2$
